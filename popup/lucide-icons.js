/** Lucide SVG icons (ISC) + Simple Icons GitHub — local copies for MV3 popup, no CDN. */
...
function svgFill(paths) {
  return `<svg ${ATTRS_FILL}>${paths}</svg>`
}
...
export const icons = {
  ...
  /* Simple Icons SiGithub — как у соседей (@icons-pack/react-simple-icons) */
  github: svgFill(
  ...
  bell: svg(
    '<path d="M10.268 21a2 2 0 0 0 3.464 0"/><path d="M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 6.326"/>',
  ),
}
...
export function iconHtml(name, className = 'icon') {
  ...
  if (!raw) return ''
  return raw.replace('<svg ', `<svg class="${className}" data-icon="${name}" `)
}
