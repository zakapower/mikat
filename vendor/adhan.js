...
const aqrabBaladResolver = (coordinates, date, latitude) => {
  const solarTime = new SolarTime(date, _objectSpread2(_objectSpread2({}, coordinates), {}, {
    latitude
  }));
  const tomorrow = dateByAddingDays(date, 1);
  const tomorrowSolarTime = new SolarTime(tomorrow, _objectSpread2(_objectSpread2({}, coordinates), {}, {
    latitude
  }));
  if (!isValidSolarTime(solarTime) || !isValidSolarTime(tomorrowSolarTime)) {
